...
func (db *DB) GetBestLifts(userAccountID types.UserAccountID) ([]types.BestLiftOverview, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&bestLiftOverview.ExerciseName, &bestLiftOverview.Repetitions, &bestLiftOverview.Weight, &bestLiftOverview.Duration); err != nil {
			return []types.BestLiftOverview{}, err
		}
